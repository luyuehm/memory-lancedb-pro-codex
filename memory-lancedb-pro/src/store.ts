...
function asTableRows<T extends object>(rows: T[]): Record<string, unknown>[] {
...
export class MemoryStore {
  ...
  private async doInitialize(): Promise<void> {
    ...
    try {
      ...
      try {
        table = await db.createTable(TABLE_NAME, asTableRows([schemaEntry]));
  ...
  async store(
    ...
    try {
      await this.table!.add(asTableRows([fullEntry]));
    } catch (err: any) {
      const code = err.code || "";
      const message = err.message || String(err);
      throw new Error(
  ...
  async update(
    ...
    return this.runSerializedUpdate(async () => {
      ...
      await this.table!.delete(`id = '${resolvedId}'`);
      try {
        await this.table!.add(asTableRows([updated]));
      } catch (addError) {
