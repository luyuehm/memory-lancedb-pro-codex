...
export function registerMemoryUpdateTool(
  ...
  api.registerTool(
    (toolCtx) => {
      ...
      label: "Memory Update",
      description:
        "Update an existing memory. Preference/entity text changes create a superseding version to preserve history; metadata-only changes stay in-place.",
      parameters: Type.Object({
          ...
          if (text && newVector) {
            ...
            if (existing) {
              ...
              if (TEMPORAL_VERSIONED_CATEGORIES.has(meta.memory_category)) {
                const now = Date.now();
                const nextStoreCategory = (category as any) || existing.category;
                const nextMemoryCategory = category
                  ? reverseMapLegacyCategory(nextStoreCategory, text)
                  : meta.memory_category;
                const factKey =
                  meta.fact_key ?? deriveFactKey(nextMemoryCategory, text);
                const nextImportance =
                  importance !== undefined
                    ? clamp01(importance, 0.7)
                    : existing.importance;
                const nextSourceSession =
                  resolveRuntimeSessionKey(runtimeCtx) ||
                  resolveRuntimeSessionKey(toolCtx) ||
                  meta.source_session;

                const newMeta = buildSmartMetadata(
                  { text, category: nextStoreCategory, importance: nextImportance },
                  {
                    l0_abstract: text,
                    l1_overview: meta.l1_overview,
                    l2_content: text,
                    memory_category: nextMemoryCategory,
                    tier: meta.tier,
                    access_count: 0,
                    ...
                    source_session: nextSourceSession,
