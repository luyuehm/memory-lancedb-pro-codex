...
`memory-lancedb-pro` 现在支持一层 A-MAC 风格的 admission control，接在 smart extraction 写路径上。

它的定位是治理层，而不是替换现有去重语义：
- 先对提取出的候选记忆打分
- 低价值候选可以在落库前直接拒绝
- 被放行的候选仍然继续走现有 downstream dedup 流程
...
可选 preset：
- `balanced` → 大多数部署的推荐默认值
- `conservative` → 更偏 precision，更少记忆进入长期存储
- `high-recall` → 更偏 recall，适合担心漏掉高价值记忆的场景
...
完整设计、五特征说明、preset 选择、审计行为和上线建议，见 [docs/admission-control.zh-CN.md](docs/admission-control.zh-CN.md)。
...
> 🛡️ admission-control 的设计、preset 和观测方式请看 [docs/admission-control.zh-CN.md](docs/admission-control.zh-CN.md)
...
| [Admission Control 指南](docs/admission-control.zh-CN.md) | A-MAC 风格治理层、preset、审计与观测 |
