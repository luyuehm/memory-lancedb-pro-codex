...
export class SmartExtractor {
     ...
     */
    async processCandidate(candidate, conversationText, sessionKey, stats, targetScope, scopeFilter) {
        ...
        if (admission?.decision === "reject") {
            ...
            await this.recordRejectedAdmission(candidate, conversationText, sessionKey, targetScope, scopeFilter, admission.audit);
            return;
        }
        // Profile always merges (skip dedup)
        if (ALWAYS_MERGE_CATEGORIES.has(candidate.category)) {
            await this.handleProfileMerge(candidate, sessionKey, targetScope, scopeFilter, vector || [], admission?.audit);
            stats.merged++;
            return;
        }
        ...
        // Dedup pipeline
        const dedupResult = await this.deduplicate(candidate, vector, sessionKey, scopeFilter);
        switch (dedupResult.decision) {
            ...
            case "merge":
                if (dedupResult.matchId &&
                    ...
                    await this.handleMerge(candidate, vector, dedupResult.matchId, scopeFilter, targetScope, dedupResult.contextLabel, admission?.audit);
                    stats.merged++;
                }
                else {
            ...
            case "supersede":
                if (dedupResult.matchId &&
                    ...
                    await this.handleSupersede(candidate, vector, dedupResult.matchId, sessionKey, targetScope, scopeFilter, admission?.audit);
                    stats.created++;
            ...
            case "support":
                if (dedupResult.matchId) {
                    await this.handleSupport(dedupResult.matchId, scopeFilter, { session: sessionKey, timestamp: Date.now() }, dedupResult.reason, dedupResult.contextLabel, admission?.audit);
     ...
     */
    async deduplicate(candidate, candidateVector, sessionKey, scopeFilter) {
        // Stage 1: Vector pre-filter — find similar memories
        const similar = await this.store.vectorSearch(candidateVector, 5, SIMILARITY_THRESHOLD, scopeFilter, { excludeInactive: true });
        if (similar.length === 0) {
    ...
    findSameTurnMemoryStoreMatch(candidate, similar, sessionKey) {
        for (const match of similar) {
            const meta = parseSmartMetadata(match.entry.metadata, match.entry);
            if (meta.source_session === sessionKey &&
    ...
    async llmDedupDecision(candidate, similar) {
        ...
        try {
            ...
            const matchEntry = typeof idx === "number" && idx >= 1 && idx <= topSimilar.length
                ? topSimilar[idx - 1]
                : topSimilar[0];
            return {
                decision,
     ...
     */
    async handleProfileMerge(candidate, sessionKey, targetScope, scopeFilter, vector, admissionAudit) {
        ...
        if (profileMatch) {
            await this.handleMerge(candidate, vector || [], profileMatch.entry.id, scopeFilter, targetScope, undefined, admissionAudit);
        }
        else {
     ...
     */
    async handleMerge(candidate, vector, matchId, scopeFilter, targetScope, contextLabel, admissionAudit) {
        ...
        try {
            ...
            await this.storeCandidate(candidate, vector || [], "merge-fallback", targetScope, admissionAudit);
            return;
        }
        // Call LLM to merge
        ...
        try {
            ...
            if (updatedEntry) {
                ...
                const finalMetadata = stringifySmartMetadata(this.withAdmissionAudit({ ...meta, support_info: updated }, admissionAudit));
    ...
    async handleSupersede(candidate, vector, matchId, sessionKey, targetScope, scopeFilter, admissionAudit) {
        const existing = await this.store.getById(matchId, scopeFilter);
        if (!existing) {
            await this.storeCandidate(candidate, vector, sessionKey, targetScope, admissionAudit);
            return;
        ...
        const invalidatedMetadata = stringifySmartMetadata(this.withAdmissionAudit(buildSmartMetadata(existing, {
            fact_key: factKey,
            invalidated_at: now,
        ...
        await this.store.update(matchId, { metadata: invalidatedMetadata }, scopeFilter);
     ...
     */
    async handleSupport(matchId, scopeFilter, source, reason, contextLabel, admissionAudit) {
        const existing = await this.store.getById(matchId, scopeFilter);
        if (!existing)
            return;
        const meta = parseSmartMetadata(existing.metadata, existing);
     ...
     */
    async handleContextualize(candidate, vector, matchId, sessionKey, targetScope, scopeFilter, contextLabel, admissionAudit) {
        const storeCategory = this.mapToStoreCategory(candidate.category);
        const metadata = stringifySmartMetadata(this.withAdmissionAudit({
            ...
            tier: "working",
            access_count: 0,
            // Keep lifecycle confidence semantics stable; admission support confidence lives in audit metadata.
            confidence: 0.7,
     ...
     */
    async handleContradict(candidate, vector, matchId, sessionKey, targetScope, scopeFilter, contextLabel, admissionAudit) {
    ...
    async storeCandidate(candidate, vector, sessionKey, targetScope, admissionAudit) {
        ...
        const storeCategory = this.mapToStoreCategory(candidate.category);
        const metadata = stringifySmartMetadata(this.withAdmissionAudit(buildSmartMetadata({
            text: candidate.abstract,
    ...
    withAdmissionAudit(metadata, admissionAudit) {
        ...
        return {
            ...metadata,
            admission_control: admissionAudit,
        };
    }
    async recordRejectedAdmission(candidate, conversationText, sessionKey, targetScope, scopeFilter, audit) {
        ...
        const excerpt = conversationText.length > REJECTION_AUDIT_EXCERPT_CHARS
            ? conversationText.slice(-REJECTION_AUDIT_EXCERPT_CHARS)
            : conversationText;
        try {
            await this.onAdmissionRejected({
                version: "amac-v1",
                rejected_at: audit.evaluated_at,
            ...
            this.debugLog(`memory-lancedb-pro: smart-extractor: rejected admission audit write failed: ${String(err)}`);
        }
    }
}
