...
export class NoisePrototypeBank {
    ...
    async init(embedder) {
        ...
        if (this.vectors.length >= 2) {
            ...
            if (sim > 0.98) {
                this.debugLog(`noise-prototype-bank: degenerate embeddings detected (pairwise cosine=${sim.toFixed(4)}), disabling noise filter`);
                this._initialized = false;
                this.vectors = [];
                return;
            }
        }
        this.debugLog(`noise-prototype-bank: initialized with ${this.builtinCount} built-in prototypes`);
    }
    /**
    ...
    learn(textVector) {
        ...
        this.debugLog(`noise-prototype-bank: learned new noise prototype (total: ${this.vectors.length})`);
    }
}
// ============================================================================
// Cosine Similarity
