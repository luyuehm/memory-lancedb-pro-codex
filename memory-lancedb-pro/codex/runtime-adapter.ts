...
export interface CodexMemoryConfig {
  dbPath?: string;
...
export interface CodexMemoryRuntimeOptions {
...
export interface MemoryHealthReport extends RuntimeSummary {
...
export interface ExtractAndStoreParams {
...
const CATEGORY_ENUM: MemoryCategory[] = [
  "preference",
  "fact",
  "decision",
  "entity",
...
function resolveEnvTemplates<T>(value: T): T {
  if (typeof value === "string") {
    return resolveTemplateString(value) as T;
  }
  if (Array.isArray(value)) {
    return value.map((item) => resolveEnvTemplates(item)) as T;
  }
...
function clampImportance(value: number | undefined, fallback = 0.7): number {
...
function resolveMaybeRelativePath(value: string | undefined, baseDir?: string): string | undefined {
  if (!value) return undefined;
...
function normalizeCategory(value: string | undefined): MemoryCategory | undefined {
  if (!value) return undefined;
  const normalized = value.trim() as MemoryCategory;
  return CATEGORY_ENUM.includes(normalized) ? normalized : undefined;
}

function pickPrimaryApiKey(value: string | string[] | undefined): string | undefined {
  if (Array.isArray(value)) {
...
function buildEnvConfig(): CodexMemoryConfig {
  ...
  if (mode === "hybrid" || mode === "vector") retrieval.mode = mode;
  ...
  if (vectorWeight !== undefined) retrieval.vectorWeight = vectorWeight;
  const bm25Weight = parseNumber(process.env.MEMORY_LANCEDB_PRO_BM25_WEIGHT);
  if (bm25Weight !== undefined) retrieval.bm25Weight = bm25Weight;
  const minScore = parseNumber(process.env.MEMORY_LANCEDB_PRO_MIN_SCORE);
  if (minScore !== undefined) retrieval.minScore = minScore;
  const hardMinScore = parseNumber(process.env.MEMORY_LANCEDB_PRO_HARD_MIN_SCORE);
  if (hardMinScore !== undefined) retrieval.hardMinScore = hardMinScore;
  const candidatePoolSize = parseNumber(process.env.MEMORY_LANCEDB_PRO_CANDIDATE_POOL_SIZE);
  if (candidatePoolSize !== undefined) retrieval.candidatePoolSize = candidatePoolSize;
...
function mergeScopes(
  base: Partial<ScopeConfig> | undefined,
  overrides: Partial<ScopeConfig> | undefined,
): Partial<ScopeConfig> | undefined {
...
function mergeConfig(base: CodexMemoryConfig, overrides: CodexMemoryConfig): CodexMemoryConfig {
  const merged = mergeDefined(
    base as Record<string, unknown>,
    overrides as Record<string, unknown>,
  ) as CodexMemoryConfig;

  if (base.embedding || overrides.embedding) {
    merged.embedding = mergeDefined(
      (base.embedding ?? {}) as Record<string, unknown>,
      (overrides.embedding ?? {}) as Record<string, unknown>,
    ) as CodexMemoryConfig["embedding"];
  }

  if (base.retrieval || overrides.retrieval) {
    merged.retrieval = mergeDefined(
      (base.retrieval ?? {}) as Record<string, unknown>,
      (overrides.retrieval ?? {}) as Record<string, unknown>,
    ) as CodexMemoryConfig["retrieval"];
  }

  if (base.llm || overrides.llm) {
    merged.llm = mergeDefined(
      (base.llm ?? {}) as Record<string, unknown>,
      (overrides.llm ?? {}) as Record<string, unknown>,
    ) as CodexMemoryConfig["llm"];
  }
...
async function readConfigFile(configPath: string): Promise<CodexMemoryConfig> {
  const raw = await readFile(configPath, "utf8");
  const parsed = JSON.parse(raw) as CodexMemoryConfig;
...
async function loadConfig(options: CodexMemoryRuntimeOptions = {}): Promise<LoadedConfig> {
  ...
  const candidatePaths = [
    ...
    .filter(Boolean)
    .map((entry) => resolve(entry as string));
...
function getConfiguredVectorDimensions(config: CodexMemoryConfig): number {
  ...
  if (model) {
    try {
      ...
      if (dimensions && dimensions > 0) return dimensions;
    }
  }
  if (dimensions && dimensions > 0) return dimensions;
...
function hasEmbeddingConfig(config: CodexMemoryConfig): config is CodexMemoryConfig & {
  embedding: EmbeddingConfig & { apiKey: string | string[]; model: string };
} {
...
function serializeEntry(entry: MemoryEntry, score?: number, sources?: RetrievalResult["sources"]) {
  return {
    id: entry.id,
    text: entry.text,
...
export class CodexMemoryRuntime {
  private readonly config: CodexMemoryConfig;
  ...
  private getScopeFilter(scope?: string): string[] | undefined {
    if (!scope) return undefined;
    const trimmed = scope.trim();
    if (!trimmed) return undefined;
    if (!this.scopeManager.validateScope(trimmed)) {
      throw new Error(
        `Invalid scope "${trimmed}". Use "global" or a built-in pattern like "project:<id>", "agent:<id>", "user:<id>", or "custom:<id>".`,
      );
    }
    return [trimmed];
  }
  ...
  private resolveScopeFilterList(scopes?: string[]): string[] | undefined {
  ...
  private getEmbedder() {
    ...
    if (!hasEmbeddingConfig(this.config)) {
      throw new Error(
        "Embedding config is missing. Add plugins/memory-lancedb-pro/config.json or ~/.codex/memory-lancedb-pro/config.json with embedding.apiKey and embedding.model.",
      );
    }
  ...
  private getLlmClient() {
    ...
    if (!apiKey) {
      throw new Error(
        "LLM config is missing. Add llm.apiKey or reuse embedding.apiKey in plugins/memory-lancedb-pro/config.json.",
      );
    }
  ...
  private getSmartExtractor() {
    if (this.smartExtractor) return this.smartExtractor;
  ...
  async health(testEmbeddings = false): Promise<MemoryHealthReport> {
  ...
  async recall(params: RecallParams) {
    ...
    if (hasEmbeddingConfig(this.config)) {
      ...
      return {
        ...
        memories: results.map((result) =>
          serializeEntry(result.entry, result.score, result.sources),
        ),
      };
    }
    ...
    return {
      ...
      memories: filtered.map((result) =>
        serializeEntry(result.entry, result.score, {
  ...
  async storeMemory(params: StoreParams) {
  ...
  async updateMemory(params: UpdateParams) {
    const hasAnyField =
    ...
    const updates: Parameters<MemoryStore["update"]>[1] = {};
  ...
  async forgetMemory(params: ForgetParams) {
    const deleted = await this.getStore().delete(
      params.id,
      this.getScopeFilter(params.scope),
    );
    return {
      deleted,
      id: params.id,
  ...
  async listMemories(params: ListParams = {}) {
    const entries = await this.getStore().list(
      this.getScopeFilter(params.scope),
      normalizeCategory(params.category),
      clampInt(params.limit, 20, 1, 100),
      clampInt(params.offset, 0, 0, 10_000),
    );
  ...
  async stats(params: StatsParams = {}) {
    const store = this.getStore();
  ...
  async extractAndStoreMemories(
    ...
    const stats = await this.getSmartExtractor().extractAndPersist(
      conversationText,
      sessionKey,
      {
        scope,
...
export async function createCodexMemoryRuntime(
  options: CodexMemoryRuntimeOptions = {},
): Promise<CodexMemoryRuntime> {
